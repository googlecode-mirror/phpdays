 ...
 *  - start generator - type path to this file (~/workbench/probus/Probus/Tools/Generator/Generator.php)
 ...
 * @license &copy; Anton Danilchenko
 * @link http://probus.googlecode.com
 * @package Tools
 * @category Probus
 * @version 0.9
 */
class Phpdays_Tool_Generator {
	/**
	 * Link to itself object.
	 *
	 * @var Probus_Tools_Generator
	 */
...
try {
	...
	$oGenerator = Phpdays_Tool_Generator::singleton();
	// start screen (introduction)
	print "\tCREATE PROJECT MODULES\n";
	print "\n";
	print "Syntax: COMMAND controllerName: action1 action2 action3\n";
	...
	print "\n";
	print "Press Enter for exit (no input module name)\n";
	...
	do {
		...
		if ('' != $sCommandLine) {
			...
			switch ($sCommand) {
				...
				case 'create':
					// create actions
					if($oGenerator->add($sController, $aCommandLineParams)) {
				...
				case 'remove':
					// backup actions
					// remove actions
					$oGenerator->del($sController, $aCommandLineParams);
					// remove controller dir (if empty)
					break;
				case 'replace':
					// backup data
					// create empty actions
					$oGenerator->add($sController, $aCommandLineParams, true);
					break;
				case 'check':
					break;
				default:
	...
	print $oEx->getMessage();
	print "\n";
}
print "Generator end work\n";
